...
contract Genesis{
    ...
    enum statusEnum {OPEN, APPROVED, REVERTED, DELETED, PAIDOUT }
    ...
    function createProject (string memory title, string memory description,string memory imageURL, uint cost,uint expiresAt ) public returns (bool){
            ...
            require(bytes(imageURL).length > 0,"ImageURL cann ot be empty");
    ...
    function updateProject(uint id, string memory title, string memory description ,string memory imageURL,uint expiresAt)public returns (bool){
        ...
        require(bytes(title).length > 0, "Title cannot be empty");
        require(bytes(description).length > 0, "Description cannot be empty");
        require(bytes(imageURL).length > 0, "IamgeURL cannot be empty");
    ...
    function deleteProject(uint id) public returns (bool){
        require(projects[id].status == statusEnum.OPEN , "Project no longer found");
        require(msg.sender == projects[id].owner, "Unauthorized Entity"  );
    ...
    function backProject(uint id) public payable returns (bool) {
        ...
        backersOf[id].push(
            backerStruct(
                msg.sender, msg.value, block.timestamp, false
                ));
    ...
    function pauOutProject(uint id) public returns (bool){
